...
/*Ques1. Company wants to fire a few of their agents' basic minimum order amount. 
...
select * from (
select agent_name, ordamt, rank() over(order by ordamt asc) rnk from (
select agent_name, sum(ord_amount) ordamt from agents a
inner join orders o 
on a.agent_code = o.agent_code
group by agent_name) tbl) tbl1
where rnk<6;



/*Ques2: Company wants to find the name of a customer from every country who has a minimum 
outstanding amount. Write a sql to find the name of the customer who has the minimum 
outstanding amount from every country. Output should have cust_country and cust_name.
Note: Data provided by an agent whose name is Mukesh is not reliable, 
so ignore the data for this agent.
*/
select * from (
select cust_country, cust_name, 
rank() over(partition by cust_country order by minamt) rnk from (
select cust_country, cust_name, min(outstanding_amt) minamt from customer 
where 
agent_code not in (select agent_code from agents where agent_name = 'Mukesh')
group by cust_country,cust_name) tbl) tbl1
where rnk=1;



/*Ques3: Write a sql query to add a new column named tot_rcv_amt in the table 
customers which contains the total receive amount for every country. 
Display all columns from the customer table along with the tot_rcv_amt column in the last.*/

select *,
sum(receive_amt) over(partition by cust_country) as tot_rcv_amt
 from customer;


/*Ques4. Company wants to change the comission for the agents' basis advance payment 
they collected. Write a sql query which creates a new column updated_commision on the 
basis below rules.
If the average advance amount collected is less than 750, the new comission will be 
0.75 times the old comission.
If the average advance amount collected is more than equal to 750 and less than 
equal to 1000, the new comission will be 0.9 times the old comission.
If the average advance amount collected is more than 1000, the new commission will 
be 1.2 times the old comission.
...
select tbl.agent_code,tbl.agent_name,tbl.avg_adv_amt, a.commission,
case 
	when tbl.avg_adv_amt < 750  then a.commission*0.75 
    when tbl.avg_adv_amt>=750 and tbl.avg_adv_amt<=1000 then commission*0.9
    else commission*1.2
end
as new_commission
from agents a inner join (
select a.agent_code, a.agent_name, avg(o.advance_amount) avg_adv_amt from agents a 
inner join orders o 
on a.agent_code = o.agent_code
group by a.agent_code,a.agent_name) tbl 
